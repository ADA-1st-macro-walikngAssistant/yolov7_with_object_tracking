...
""" 바운딩 박스 그리기 """
def draw_boxes(img, bounding_box, identities=None, categories=None, confidences=None, names=None, colors=None):
    for i, box in enumerate(bounding_box):
        ...
        if not opt.nobbox:  # opt.nobbox: means
...
def detect(save_img=False):
    ...
    imgsz = check_img_size(imgsz, s=stride)  # 이미지 사이즈 체크
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if view_img:
                ...
                cv2.waitKey(1)  # 1 밀리초(milliseconds)
